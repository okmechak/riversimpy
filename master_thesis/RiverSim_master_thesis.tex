...
\begin{document}
  ...
  \chapter{Rozwiązanie numeryczne i struktura oprogramowania}
    ...
    Metoda elementów skończonych wymaga wyznaczenia geometrii obszaru, następnie jej dyskretyzacji, utworzenia równań liniowych, rozwiązania równań, zintegrowania współczynników rozszerzenia $a_i$ w szereg. Powyższe kroki tworzą strukturę oprogramowania \ref{program_workflow}.
    ...
    % В реальних середовищах еволюції русла річок даного типу, ми можемо зустріти цілий ряд чинників, котрі мають вплив на поле підземних вод. Наприклад, це можуть бути гори звідки регулярно надходить потік води, озеро котре може слугувати кінцевою точкою або початком річки. Мережа річки теж має вплив на поле підземних вод і багато інших чинників. Ця довільність геометрії продемонстрована на малюнку \ref{border_examples}. І що б охопити всю цю довільність геометрії, було створено цілий ряд класів і функцій. Деякі найголовніші класи і функції: Point, Polar, Line, Boundary, BoundaryConditions, Sources, Branch, Rivers, Region і функція BoundaryGenerator.
      W rzeczywistych środowiskach ewolucji tego typu koryt rzecznych możemy napotkać szereg czynników, które mają wpływ na pole wód gruntowych. Na przykład mogą to być góry, z których regularnie wypływa strumień wody, jezioro, które może służyć jako punkt końcowy lub początek rzeki. Sieć rzeczna ma również wpływ na pole wód gruntowych i wiele innych czynników. Ta arbitralność geometrii jest pokazana na rysunku \ref{border_examples}.
      ...
      \begin{figure}[H]
        ...
        \label{border_examples}
      \end{figure}

      Aby pokryć całą tę arbitralność geometrii, stworzono szereg klas i funkcji\ref{geometria}. Niektóre z najważniejszych klas i funkcji to Point, Polar, Line, Boundary, BoundaryConditions, Sources, Branch, Rivers, Region i funkcja BoundaryGenerator.
      ...
      Głównie geometrie możemy podzielić
        ...
        Najbardziej dynamiczną cześcią granicy jest sieć rzeczna. Z każdym krokiem symulacji sięć rośnie o krok proporcionalny do $a_1^{\eta}$. Przy małych współczynnikach $a_1$ i $a_2$ otrzymamy prosty brzeg z mała krzywizną, a nadmiarową deskretyzacje co w swoją kolej zwiekszy czas rozwiązywani. Dla tego była wrpowadzony algorytm wygładzenie linii.

        Dany algorytm iteruję po punktach brzegu  zaczynając od pewnej długości zdefiniowaną parameterem \textit{ignored distance}. Dalej są wyliczane i sumowanę kąty pomiędzy sąsiednimi segmentami. Dopóki sumaryczny kąt jest poniżej kątu definiowanego przez parameter \textit{min degree}.\par

        Każdy punk brzegu może być początkiem rozwoju sieci rzecznej. Współrżędne źródeł są zadawne przez strukturę danych \textit{Source}. \textit{Source} zawiera informacje o nazwie źródła, o konkretnym brzegu(o ile brzeg może składać się z dowolnej ilości poligonów) i o pozycji współrzędnej na brzegu(zaczynając od zera). 

        Wzrost rzeki zaczyna sie od brzegu i jest perpendycularny  

        \begin{figure}[H]
          \centering
          \includegraphics[width=1\textwidth]{figs/source_dirrection.jpg}
          % Демонстрація впливу напрямку лінії на напрям перпендикуляра. У випадку напрямку за годинниковою стрілкою(1), напрямок буде назовні. Проти годинникової стрілки(2) - до середини. 
          \caption {Wykazanie wpływu kierunku linii na kierunek prostopadłej. W przypadku kierunku zgodnego z ruchem wskazówek zegara (1) kierunek będzie na zewnątrz. Przeciwnie do ruchu wskazówek zegara (2) - do środka.}
          \label{source_dirrection}
        \end{figure}

        %При задаванні початкової координати джерелла не вказується його напрямок, лише позиція. В даному випадку ріст русла річки починається з ерозії при самому березі і є перпендикулярним до лінії берегу в даному пункті. Кут перпендикулярної лінії в даній точці границі визначається автоматично з точність до $\pi$. Щоб позбутися цієї не точності $\pi$ при задаванні границі теж врахомується напрям пунктів, а саме напрям вибирається з лівою сторони від напрямку заданого пунктами на границі.
        Przy określaniu początkowej współrzędnej źródła nie jest wskazywany jego kierunek, a jedynie jego położenie. W tym przypadku rozrost koryta zaczyna się od erozji na samym brzegu i jest w tym miejscu prostopadły do linii brzegowej. Kąt prostej prostopadłej w danym punkcie granicy wyznaczany jest automatycznie z dokładnością $\pi$. Aby pozbyć się tej $\pi$ niedokładności, kierunek punktów jest również brany pod uwagę podczas ustawiania granicy, a mianowicie kierunek jest wybierany od lewej strony kierunku określonego przez punkty na granicy.
        ...
        %          t_LineList lines;
        %          void Append(Boundary& boundary);
        %          void ReplaceElement(t_vert_pos vertice_pos, Boundary& boundary);
        %          void FixLinesIndices(bool is_closed_boundary = false);
        %          Boundary generateSmoothBoundary(double min_degree, double ignored_distance);
        %          bool operator==(Boundary& simple_boundary) const;
        %  };
        %\end{lstlisting}

        Brzeg jest zdefiniowany przez 
