...
\begin{document}
  ...
  \begin{abstract}
    ...
    Networks naturally appear everywhere in nature. An electrostatic discharge between two charged areas generates a current that forms a network of channels. As the tree grows, it forms an extensive network of branches to collect solar energy efficiently over many years. Also, the development of the vascular network is controlled by the concentration of oxygen in the tissue.
    ...
    At this stage, you can already see a common feature of all processes, namely the presence of interdependent fields and matter moving in the local direction of the maximum area. Therefore, many network growing processes can be reduced to a single mathematical formalism.\par
    ...
    In this work, we focus on the creation of a Python library that helps to study the development of a network in the Laplace field using the example of the development of a river network. First, we describe the mathematical model of the development of the river network in the Laplace field. Then we describe the riversim library, its algorithms and show a simple step-by-step way of using the program. Then we give examples of the convergence of the solution and show several results.\par
  ...
  \chapter{Theoretical model}
    ...
    However, if the soil is sufficiently permeable, water can soak into the ground instead of running off (Fig. \ref{pow_wsiakanie}). Water first accumulates in an underground reservoir and then flows out of the river source and banks under the influence of hydrodynamic pressure. This process erodes the soil near the source.
    ...
    \begin{wrapfigure}{r}{0.5\textwidth}
      ...
      \caption{Illustration of the growth of the river through erosion processes occurring in the vicinity of the sources.}
      \vspace{0pt}
      \label{zrodlo}
    \end{wrapfigure}
    ...
    This erosion in the vicinity of the source, which occurs in the opposite direction to the flow, is responsible for the expansion and bifurcation of the river, the general evolution of the river network.
    ...
    The first quantitative model describing the evolution of such rivers was presented in 1980 by Dunne \cite{dunne1980formation}. More detailed models have been proposed by Daniel Rothman's group at the Massachusetts Institute of Technology (Refs.~\cite{petroff2012four, devauchelle2012ramification}). The authors linked the height of the groundwater level with the direction and speed of the river's growth.
    ...
    \section{Underground waters}
      ...
      This equation describes the filtration velocity ($\vec{u}$) of a fluid with a dynamic viscosity of $\mu$ through a porous medium (e.g. sand or rock) with a permeability of $\kappa$ under a pressure of $p$.
      ...
      To calculate how much water flows through a particular point in the $xy$ plane where the network grows, we simply integrate Darcy's velocity in the given $xy$ coordinates along the $z$ horizontal axis (height of the tank):
      ...
      Now we use the Dupuit \cite{dupuit1863etudes} assumption that groundwater flows horizontally. In addition, we assume that the pressure causing the flow is simply hydrostatic pressure $p = \rho g h $ ($\rho$ -- density of water, $g$ -- gravitational acceleration, $h$ -- height. Table of groundwater). For such a pressure, $\nabla p$ is independent of the $z$ coordinate, and the above integral simplifies to the product of the filtration velocity $\vec{u}$ and the height of the water column $h(x, y)$ at a given location:
      ...
      where the potential was entered $\phi(x,y)$:
      ...
      Finally, we derive the equations for the $\phi$ area from the continuity equation, taking into account precipitation (described by the average volume of precipitation per unit time and area, $P$). The mass change of the fluid column ($M$) enclosed in the $S$ area can be described as follows:
  ...
  \section{Solving Laplace's equation}\label{chapter:solving}
    ...
    In polar coordinates (where the direction of the tangent to the finger at the vertex sets $\theta = 0$), the field near the vertex can be extended \cite{derrida1992needle , petroff2013bifurcation}:
    ...
    Eq. \eqref{phi_polar} is also valid for Poisson fields, because in the vicinity of the vertex the contribution of the flux from the source term is negligible compared to the flux from areas far from the vertex. For each of the leading coefficients $a_i$ in the equation \eqref{phi_polar} we can find a physical interpretation.
    ...
    The $a_2$ coefficient is related to the asymmetry of the field around the vertex. With positive $a_2$, the field flux is greater on the right side of the vertex, and with negative $a_2$ - on the left side. However, the finger grows in the direction of the greatest flux, and in effect grows in such a way that $a_2$ always vanishes (the principle of local symmetry \cite{cohen2015path}).
    ...
    Finally, $a_3$ is combined with the second derivative of the field. If we focus again on the small circle of radius $r_0$ around the tip and examine the field as a function of the angle $\theta$, we see that given $a_1>0$ and $a_2=a_3=0$ the field has a single maximum at $\ theta = $0. The smaller the $a_3$ factor, the flatter the field maximum around the tip. Finally, when the second derivative of $\phi$ disappears, which corresponds to the negative value of $a_3/a_1$:
    ...
    this single maximum becomes two maximums in $\pm \theta_0$. Note that the threshold value $a_3/a_1$ depends on the distance of $r_0$ from the tip where the field is analyzed. For infinitesimals $r_0$ the profile of the field is always symmetric with one maximum at $\theta=0$.
  ...
  \chapter{Numerical solution and software structure}
    ...
    \hspace{1em} \=
    The groundwater field is shaped by erosion in the vicinity of the source and, as a result, the dynamics and development of the network. From the mathematical model for the description of river networks, we obtain equations for the description of groundwater in the form of the Laplace \ref{laplace} (or Poisson \ref{poisson}) equation. The field near the source is approximated by the coefficients $a_i$ (\ref{a1} \ref{a2} \ref{a3}). Integrating the coefficients as well as solving in turn requires numerical methods. In general, the Laplace equation can be solved by numerical methods, and in particular, the finite element method was used in this work. \\ \indent
    ...
    In previous projects, solutions for the problem of the evolution of the river network have already been presented. One of the solutions was written using Freefem++ and Matlab. Another, with the help of conformal transformations. However, in order to obtain even larger river networks, a more optimal solution was needed. The second requirement was to use only technologies with open source code, because not everyone has the opportunity to purchase Matlab. The choice fell on C++\cite{Stroustrup1997} and Python. After all, these programming languages are among the most widespread, and also contain many open source libraries, which makes it easier to write programs. C++ -- on the one hand, it is very fast, but demanding to write. C++ contains the implementation of a part of the program that is complex in calculations, such as algorithms for working with geometry, mesh generation, and solving differential equations. Python\cite{python3} -- is less computationally efficient, but much faster in software development and provides an interactive scripting tool -- Jupyter.
    ...
    RiverSim includes some additional algorithms for reversible backward and forward evolution, and besides RiverSim and riversimps contain the same algorithms, in the following description we will mainly focus on riversimps.
    ...
    \section{Geometry}
      ...
      \begin{figure}[H]
        ...
        \caption{Several examples of a boundary: a bank with two selected sources (1), a boundary containing a boundary (for example, a lake) (2), a river basin approximated by a broken line and a partially reflected river (4), two rectangles with different widths, not topologically related (5).}
      ...
      And in order to cover all this arbitrariness of geometry, a number of classes and functions were created \ref{GeometryClasses}. Some of the most important classes and functions are Point, Polar, Line, Boundary, BoundaryConditions, Sources, Branch, Rivers, Region, and the BoundaryGenerator function. We will consider examples of the use of these functions below.
      ...
      \begin{figure}[H]
        ...
        %Ilustracja kolejności kroków przy definicji regionu. Definicja wszystkich punktów i zapisanie ich do oddzielnychlist t\_PointList jeżeli należą do różnych brzegów (1). Definicja linii i indeksów i też przepisanie ich do oddzielnych list t\_LineList (2). Definicja punktów źródłowych na granicy w klasie Source (3). Definicja warunków brzegowych (4).
        \caption{Illustration of the sequence of steps in defining a region. Define all points and store them in separate lists t\_PointList if they belong to different edges (1). Definition of lines and indexes and rewriting them to separate lists t\_LineList (2). Definition of source points on the boundary in the Source class (3). Definition of boundary conditions (4).}
      ...
      First, we specify an ordered set of coordinates using the Point class and lines using the Line class to determine the boundary. In the program, the boundary is represented by the Boundary class. There can be several such borders. A group of boundaries assigned a unique index forms a region represented by the Region class. The border is a closed line, in other words, a polygon. There are no limits on the number of borders in the region. The border can also contain a completely different border, but crossing lines is not allowed. To create a hole in a border, you need to define a coordinate within this border and assign it to the list of holes stored in the Region class. The next step is to determine the places from where the growth of the river network will take place. Such places can be every point of the border and there can be an unlimited number of sources (practically limited by the total number of coordinates of the borders). The Source class was created to store information about the initial coordinates of the sources. This class stores the unique number of each source, the number of the boundary on which it is located, and the number of the coordinate on this boundary.
      ...
      After determining the region and the location of the sources, we can now proceed to the construction of the geometry of the river. All river geometry data and algorithms are contained in the Rivers class. Rivers contains a unique number for each river, which has the same meaning as the source number in Sources. To initialize the Rivers class, you need to convert the data from the Sources class into coordinates and direction. For this, the GetSourcesIdsPointsAndAngles function of the Region class serves as an argument, which has Sources as its argument. As a result, we will get a set of river indices and their corresponding coordinates and the direction perpendicular to the border. To determine the exact direction of the perpendicular, the direction of the coordinate is important when setting the border. It is assumed that the perpendicular will be on the left side of the coordinate direction. Therefore, if you pay attention to the region where there is a hole in the center, then the outer border has the opposite direction of coordinate numbering relative to the inner one.
      ...
      Since the river network contained in the Rivers class rests in its implementation on the Boundary class, we can set unique boundary conditions for each segment of the river. For example, it is possible to take into account the "resistance" of the river bed and the increase in the height of groundwater along it.
      ...
      \begin{wrapfigure}{r}{0.5\textwidth}
        ...
        \end{center}
        %Ilustracja algorytmu zmniejszenia ilości punktów brzegu.
    ...
    \section{Mesh generation}
      ...
      \begin{figure}[H]
        ...
        \caption{All banks that are contained in the Region class, as well as river banks in the Rivers class merge into one closed bank (1). With the help of a Triangle, a triangular mesh (2) is generated on the basis of one closed border. A triangle mesh is converted into a rectangle mesh with the help of the Tethex library (3).}
      ...
      The Deal.II\cite{dealII94} solver requires a mesh with square elements. For this, we use the Tethex \cite{tethex} C++ library, which converts triangular grid elements into quadrilaterals, dividing one triangular element into three square ones, thereby increasing the number of grid elements by three times.
    ...
    \section{Solution of groundwater field}
      ...
      We're going to solve a simple version of the Poisson equation in our area with a non-zero right-hand side.
      ...
      We will only consider the special case $f(x)=P$.
      ...
      The test function $u$ must satisfy the same kind of boundary conditions (in the mathematical sense: it must come from the tangent space of the set in which we are looking for a solution), so at the boundary $u=0$ we consequently get the next appearance of the weak form:
      ...
      \begin{itemize}
        ...
        \item A finite element describing the shape functions we want to use in the reference cell (unit square $[0,1]^2$). Usual Lagrangian functions;
      ...
      Finally, we get the equation for the approximate solution of $\Phi$ to
      ...
      where the $A$ matrix and the right side of the $F$ are defined as:
      ...
      \paragraph{Adaptive mesh}
        ...
        \begin{figure}[H]
          ...
          \caption{Mesh elements are divided in those places where the solution error is the greatest. Near the source, the field behaves as $\frac{1}{\sqrt[2]{r}}$.}
        ...
        Outside of this structure, however, there are many ways to achieve this goal. They fundamentally differ in how accurately one mesh is generated from the previous one. For a triangle mesh, the triangle selected for refinement is split in two by inserting one new edge from the center of the longest edge to the opposite vertex. Of course, the midpoint from the longest edge has to be balanced somehow by refining the cell on the other side of that edge (if any). This method doesn't work for quads, at least it's not easy. The reason is that the transition elements created from the quadrilateral neighbors of the quadrilateral cell to be refined would be triangles, and we don't want that. Therefore, the approach to adaptability is chosen in Deal.II is to use grids where adjacent cells can differ in the level of refinement by one. This causes nodes on cell interfaces to belong to one side but are unbalanced to the other. The common term for these is "hanging knots". If we refine the mesh globally, the error will fall as
        ...
        (Since $h_K\le h_\text{max}$, this formula immediately implies the previous one if you simply take the mesh size from the sum.) This formula implies that it is not necessary that the largest cell be small, but that these cells actually need to be small where $\| \nabla^{p+1} \phi \|_K$ is big! In other words: The grid only really needs to be exact if the solution has large changes, as indicated by the $ p+1$ derivative.
        ...
        Of course, this a priori estimate isn't very useful in practice, since we don't know the exact solution to $\phi$, and therefore we can't compute $\nabla^{p+1} \phi$. But we can compute numerical approximations of $\nabla^{p+1} \phi$ based only on the discrete solution $\phi_h$.
    ...
    \section{Integrating series parameters}
      ...
      \begin{figure}[H]
        ...
        \caption{Illustration of the distribution of the field in the vicinity of the source into $a_n$ coefficients.}
      ...
      It has already been shown above that erosion is greatest in places where the field gradient is greatest. Such a place with the greatest gradient is around the sources. In polar coordinates, the field near the vertex can be extended using the equation \ref{series_params_integration}. Where each parameter $a_n$, $n \epsilon [1, 3]$ has a physical interpretation (\ref{circle}, \ref{a3a1}) and is associated with growth rules (\ref{velocity}). The coefficients $a_n$ are derived from the integral form \ref{a1}, \ref{a2} and \ref{a3}. To reduce the numerical noise, we average the integral forms along the $r$ radius:
      ...
      Where $\omega(r)$ the weight function is:
      ...
      \begin{mintedbox}{c++}
        ...
        In riversimpy, the integrate (or integrate\_new) method of the Solver class is used to calculate the $a_i$ coefficients:
  ...
  \chapter*{Summary}
    ...
    In this master's thesis, we considered the development of the spatial network using the example of the evolution of the river network. First, we represented a theoretical model with a focus on groundwater and erosion in the vicinity of sources. Simplification of the one-dimensional river was introduced. It was shown how to describe the field of groundwater $\phi(x, y)$ in the vicinity of the source using the parameters of the $a_i$ distribution. Then it was shown how the parameters $a_i$ can be interpreted and how they are related to the dynamics of the network development.\par
    ...
    Next, we offered our numerical solution to this complex problem of the interdependence of the network and the field under whose influence it develops in the Python library -- riversim. The structure of the library was described in detail with examples of its use. How to set arbitrary initial geometry. How to set the initial position of the sources from which the river will develop. In which data structure is the river network stored and how to work with it. How to set boundary conditions and what types they are present in the program. How to generate a mesh from the initial geometry using the Tethex and Triangle C++ libraries. After that, how to get the groundwater field using the FEM method, which is implemented in the Deal.II library. How to calculate $a_i$ parameters using a numerical integral.\par
    ...
    \section*{Acknowledgements}
      ...
      I would like to thank Olivier Devauchelle from Institut de Physique du Globe, Paris and Hansjörg Seybold from ETH Zurich for sharing the code of river network growth simulation in FreeFEM++ which helped me to came out with my own solution.
