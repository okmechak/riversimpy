...
\begin{document}
  ...
  \chapter{Rozwiązanie numeryczne i struktura oprogramowania}
      ...
      Z matematycznego modelu dla opisu sieci rzecznych otrzymujemy rownania dla opisu wód podziemnych w postaci równania Laplacea \ref{laplace}(lub Poissona \ref{poisson}). W ogólnym przypadku równanie Laplace można rozwiązać metodami numerycznymi, a w szczegolności w tej pracy skorzystano z metody skończonych elementów.\par

      Pole wód podziemnych, którę kształtuję erozje w oklicy żródła i jak skutek dynamikę i rozwój sięci, jest uproszczane do współcznynników $a_i$ (\ref{a1} \ref{a2} \ref{a3}) całkowanie których też w swoją kolej wymaga metod numerycznych.\par
      
      Jednem z celów projektu było otrzymanie oprogramowania które by było szybkim w wyliczaniu powyższych zagadnień, a tak że szybkim w prototypowaniu. Wybór padł na C++\cite{Stroustrup1997} i Python. C++ -- z jednej strony jest bardzo szybkim, ale wymagający do napisania, w nim realizowano części programu które są czasochłonne. Python -- ma mniejsża wydajnoś oblicznenia,  ale o wiele szybszy w rozwoju oprogramowaniu. TODO: \cite{skjdfkjdsfh}   
      ...
      \begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{figs/ProgramWorkflow.jpg}
        \caption {Schemat działania programu RiverSim.}
    ...
    \section{Dyzajn i główne cele oprogromawonia. Szybkość działania a szybkość prototypowenia.}
      ...
      Metoda FEM na początku wymaga zdefiniowanie domeny, potem jej descrtetyzacji, tworzenie równań liniowych, rozwiązanie równań i po otrzymaniu wyniku możemy kontynuować obliczenie, na przykład całkowanie współcznynników $a_i$. Powyższe kroki i kształtują strukturę oprogramowania. 
