...
\begin{document}
  ...
  \begin{abstract}
    ...
    At this stage, you can already see a common feature of all processes, namely the presence of interdependent fields and matter moving in the local direction of the maximum field flux. Therefore, many network growing processes can be reduced to a single mathematical formalism.\par
  ...
  \chapter{Theoretical model}
    ...
    However, if the soil is sufficiently permeable, water can soak into the ground instead of running off (Fig. \ref{pow_wsiakanie}). Water first accumulates in an underground reservoir and then flows out of the river source and boundaries under the influence of hydrodynamic pressure. This process erodes the soil near the source.
  ...
  \chapter{Numerical solution and software structure}
    ...
    RiverSim includes some additional algorithms for reversible backward and forward evolution, and besides RiverSim and riversimpy contain the same algorithms, in the following description we will mainly focus on riversimpy.
    ...
    \section{Geometry}
      ...
      \begin{figure}[H]
        ...
        \caption{Several examples of a boundary: a boundary with two sources (1), a boundary containing a hole (for example, a lake) (2), a river basin approximated by a broken line and a partially reflected river (4), three rectangles with different widths, not topologically related (5).}
      ...
      And in order to cover all this arbitrariness of geometry, a number of classes and functions were created (Pic. \ref{GeometryClasses}). Some of the most important classes and functions are Point, Polar, Line, Boundary, BoundaryConditions, Sources, Branch, Rivers, Region, and the BoundaryGenerator function. We will consider examples of the use of these functions below.
    ...
    \section{Solution of groundwater field}
      ...
      We're going to solve a simple version of the Poisson equation in our area with a non-zero right-hand side \ref{poisson}(We will only consider the $\frac{P}{k} = f$.).
